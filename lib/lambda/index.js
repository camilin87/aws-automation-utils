module.exports = function(lambdaHelperFn){
    if (!lambdaHelperFn){
        lambdaHelperFn = require('./lambdaHelper')
    }
    ...
    return {
        updateEnvironment: async (config) => {
            var result = false
            try{
                const lambda = lambdaHelperFn(config.region)

                const functionConfiguration = await lambda.readConfiguration(config.functionName)
                functionConfiguration.Environment = functionConfiguration.Environment || {}
                functionConfiguration.Environment.Variables = functionConfiguration.Environment.Variables || {}
        ...
        updateStatus: async (config) => {
            ...
            try{
                ...
                await lambda.readConfiguration(config.functionName)
